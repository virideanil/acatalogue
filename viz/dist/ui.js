...
const BASELINE_NAMES = { population: "population", land_area: "land area", equal: "equal shares" };
/** Above parity (interval entirely above 1x), below (entirely below), or cannot tell (it spans 1x). */
function rrState(lo, hi, rr) {
    if (rr === null)
        return "none";
    if (lo !== null && lo > 1)
        return "over";
...
const RR_SHORT = { over: "above", under: "below", neutral: "spans 1×", none: "no baseline" };
function ratioText(r) {
    if (r === 0)
        return "0×";
    const num = (v) => (v >= 10 || Math.abs(v - Math.round(v)) < 1e-9 ? String(Math.round(v)) : v.toFixed(1));
 ...
 */
function rrChart(level, baseline, tip) {
    const W = 340;
    const labelW = 122;
    const x0 = labelW + 8;
    const x1 = W - 10;
    ...
    for (const g of rows) {
        ...
        if (!v || v.rr === null || v.rr === 0)
            continue;
        if (v.rr_lo !== null && v.rr_lo > 0)
            lo = Math.min(lo, Math.log2(v.rr_lo));
        else if (v.log2_rr !== null)
            lo = Math.min(lo, v.log2_rr);
        if (v.rr_hi !== null && v.rr_hi > 0)
            hi = Math.max(hi, Math.log2(v.rr_hi));
    }
    ...
    const x = (v) => x0 + ((Math.min(hi, Math.max(lo, v)) - lo) / span) * (x1 - x0);
    ...
    rows.forEach((g, k) => {
        ...
        const show = () => {
            tip.replaceChildren(h("strong", undefined, v.rr === null || v.rr === 0 ? (v.rr === null ? "no baseline" : "none tagged") : ratioText(v.rr)), h("span", undefined, g.label), h("span", undefined, `share ${(g.share * 100).toFixed(1)}% (95% ${(g.share_lo * 100).toFixed(1)}–${(g.share_hi * 100).toFixed(1)}%)`), h("span", undefined, `${BASELINE_NAMES[baseline] ?? baseline} share ${v.baseline === null ? "unknown" : `${(v.baseline * 100).toFixed(1)}%`}`), h("span", undefined, RR_READING[state]));
            tip.hidden = false;
        };
...
export function claimValue(c) {
    if (c.snak_type === "novalue")
        return "no value";
    if (c.snak_type === "somevalue")
        return "unknown value";
    if (c.object)
        return c.object_label || c.object;
    let v = null;
    ...
    if (v !== null && typeof v === "object") {
        ...
        if (typeof o.time === "string") {
            ...
            if (m) {
                ...
                if (p >= 11)
                    return bce ? `${year}, ${m[3]}-${m[4]}${cal}` : `${year}-${m[3]}-${m[4]}${cal}`;
                if (p === 10)
                    return bce ? `${year}, month ${m[3]}${cal}` : `${year}-${m[3]}${cal}`;
                if (p === 9)
                    return year;
        ...
        if (typeof o.text === "string")
            return `${o.text}${typeof o.language === "string" ? ` (${o.language})` : ""}`;
...
export class Ui {
    ...
    pauseButton = byId("pause", HTMLButtonElement);
    allLinks = byId("all-links", HTMLInputElement);
    treePanel = byId("tree-panel", HTMLDetailsElement);
    tree = byId("tree", HTMLUListElement);
    treeModel = null;
    ...
    constructor(cb) {
        ...
        this.treePanel.addEventListener("toggle", () => {
            if (!this.treePanel.open)
                return;
            this.renderTreeRoots();
            if (this.pendingReveal >= 0)
                this.revealInTree(this.pendingReveal);
        });
    ...
    renderTreeRoots() {
        ...
        if (!m || this.tree.childElementCount > 0)
            return;
    ...
    expand(li, open) {
        ...
        let group = li.querySelector(":scope > ul");
        if (open && !group) {
            const m = this.treeModel;
    ...
    visibleItems() {
        return [...this.tree.querySelectorAll('li[role="treeitem"]')].filter((li) => !li.closest("ul[hidden]"));
    }
    focusItem(li) {
        for (const x of this.tree.querySelectorAll('li[tabindex="0"]'))
            x.tabIndex = -1;
        li.tabIndex = 0;
        li.focus();
    }
    markSelected(li) {
        for (const x of this.tree.querySelectorAll('li[aria-selected="true"]'))
        ...
        switch (e.key) {
            ...
            case "ArrowRight":
                if (li.getAttribute("aria-expanded") === "false")
                    ...
                    const first = li.querySelector(':scope > ul > li[role="treeitem"]');
                    if (first)
    ...
    revealInTree(i) {
        ...
        let container = this.tree;
        let li = null;
        for (let k = 0; k < path.length; k++) {
            li = container.querySelector(`:scope > li[data-i="${path[k]}"]`);
            if (!li)
                return;
            if (k < path.length - 1) {
                this.expand(li, true);
    ...
    showNode(rec, inField, offlineNote) {
        ...
        parts.push(this.collapsible("Mappings", rec.mappings.length, () => {
            ...
            for (const m of rec.mappings) {
                ...
                if (m.decided_by === "AI agent")
                    li.append(h("p", "badge agent", "Proposed by an AI agent; no person has reviewed it yet."));
                else if (m.decided_by === "human")
                    li.append(h("p", "badge human", `Decided by ${m.reviewer}.`));
                else if (m.decided_by === "unknown")
                    li.append(h("p", "badge", `Decided by ${m.reviewer}; the data does not say whether a person or a machine.`));
                else if (m.decided_by)
                    li.append(h("p", "badge", "Authored in the seed files."));
                if (m.method)
                    li.append(h("p", "meta", `method: ${m.method}${m.note ? ` · ${m.note}` : ""}`));
                ul.append(li);
            }
            return ul;
        }, rec.mappings.length > 0 && rec.mappings.length <= 6));
        if (rec.sources.length > 0) {
            parts.push(this.collapsible("In other sources", rec.sources.length, () => {
                const ul = h("ul", "rows");
        ...
        if (rec.reviews.length > 0) {
            parts.push(this.collapsible("Reviews", rec.reviews.length, () => {
                const ul = h("ul", "rows");
                for (const r of rec.reviews) {
                    ...
                    if (r.rationale)
                        li.append(h("p", "excerpt", r.rationale));
                    ul.append(li);
                }
                return ul;
            }, true));
        ...
        if (!offlineNote) {
            ...
            const claimRows = (list) => {
                ...
                for (const c of list) {
                    ...
                    if (meta.length > 0)
                        li.append(h("span", "meta", ` ${meta.join(" · ")}`));
    ...
    renderKey(lens, legend, showSemantic) {
        ...
        if (lens === "structure") {
            ...
            for (const t of [0, 0.25, 0.5, 0.75, 1])
                ticks.append(h("span", undefined, fmt.format(Math.round(Math.expm1(lo + t * (hi - lo))))));
    ...
    showAudit(a, offline, inField) {
        ...
        if (a.baseline)
            out.push(this.baselineSection(a.baseline, idCell, table));
    ...
    baselineSection(b, idCell, table) {
        ...
        const render = () => {
            ...
            for (const [state, text] of [["over", "above parity: the whole interval is above 1×"], ["under", "below parity: the whole interval is below 1×"], ["neutral", "cannot tell: the interval includes 1×"]]) {
            ...
            if (d)
                lines.push(`Jensen–Shannon divergence ${d.jsd_bits.toFixed(3)} bits (bootstrap 95% ${d.jsd_lo?.toFixed(3) ?? "?"}–${d.jsd_hi?.toFixed(3) ?? "?"}), ${d.exceeds_null ? "beyond" : "within"} what random sampling from the baseline gives (95th percentile ${d.null_p95?.toFixed(3) ?? "?"}). Gini of the ratios ${d.gini_rr?.toFixed(2) ?? "?"}.`);
            if (cov)
                lines.push(`Baseline: World Bank ${this.rrBaseline === "population" ? "population" : "land area"} ${cov.as_of}; ${fmt.format(cov.with_value)} of ${fmt.format(cov.areas)} UN M49 areas have a value.`);
            if (this.rrBaseline === "equal")
                lines.push("Equal shares: every group gets 1/K, whatever its size.");
        ...
        controls.append(select("Baseline", [["population", "Population (2024)"], ["land_area", "Land area (2023)"], ["equal", "Equal shares"]], this.rrBaseline, (v) => {
            this.rrBaseline = v;
            render();
        ...
        if (b.sibling_flags.length > 0) {
            ...
            sec.append(table("Sibling parity: a subtree at least 3× its siblings' median (a look, not a verdict)", ["Concept", "Size", "Median"], b.sibling_flags.slice(0, 15).map((f) => [who(f), String(f.subtree), String(f.median)]), [false, true, true]));
        }
        const facts = [];
        if (b.evidence)
            facts.push(`${fmt.format(b.evidence.sourced)} of ${fmt.format(b.evidence.statements)} current Wikidata statements about these concepts cite a source; for the hierarchy statements (instance of, subclass of, part of, facet of) ${fmt.format(b.evidence.hierarchy_sourced)} of ${fmt.format(b.evidence.hierarchy_statements)}.`);
        if (b.attention)
            facts.push(`Attention: median ${b.attention.median ?? "?"} Wikipedia language editions without the bot-generated ${b.attention.excluded.join(" and ")} editions (${b.attention.median_all ?? "?"} with them; ${fmt.format(b.attention.changed)} concepts change).`);
