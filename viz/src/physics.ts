...
export type EdgeKind = "broader" | "related" | "mapping" | "semantic";
export const EDGE_KINDS: readonly EdgeKind[] = ["broader", "related", "mapping", "semantic"];
export const KIND_BROADER = 0;
export const KIND_RELATED = 1;
export const KIND_MAPPING = 2;
export const KIND_SEMANTIC = 3;
export const KIND_CODE: Readonly<Record<EdgeKind, number>> = {
...
/** The parts of a graph node the physics needs (a structural subset of the API node). */
...
export interface SpringSpec {
  ...
  /** Stiffness per unit weight (force per unit length; multiplied by the edge's w). */
  readonly k: number;
  /**
   * Stretch beyond which the tension stops growing (a Huber spring: quadratic energy
   * near rest, linear far out), so a few long cross-links cannot tear clusters apart.
   */
...
export interface PhysicsConfig {
  ...
  readonly dt: number;
  /** Most steps one frame may run (spiral-of-death clamp). */
  readonly maxStepsPerFrame: number;
  /** Longest frame time fed into the accumulator, seconds. */
  readonly maxFrameSeconds: number;
  /** Wall-clock budget for physics per frame, ms; the backlog is dropped past it. */
  readonly frameBudgetMs: number;
  /** Viscous damping rate gamma (1/s): dv/dt = ... - gamma v. */
  readonly damping: number;
  /** Velocity clamp, world units per second. */
  readonly maxSpeed: number;
  /** Repulsion constant: force = k qi qj r / (r^2 + eps^2). */
  readonly repulsion: number;
  /** Softening length eps, world units. */
  readonly softening: number;
  /** Barnes-Hut opening angle. */
  readonly theta: number;
  /** Repulsion fades smoothly to zero between these radii (Infinity disables the cutoff). */
  readonly cutoffInner: number;
  readonly cutoffOuter: number;
  /** Centering gravity per unit mass (1/s^2): a = -g x. */
  readonly gravity: number;
  readonly springs: Readonly<Record<EdgeKind, SpringSpec>>;
  readonly ring: {
    /** Scheme whose roots sit on the inner ring. Others go to the outer ring. */
    readonly primaryScheme: string;
    /** Radial spring per unit mass toward the ring radius (1/s^2). */
    readonly k: number;
    /** Extra mutual repulsion between roots on the same ring. */
    readonly rootRepulsion: number;
    readonly rootSoftening: number;
    /** Area one particle needs, world units^2 (sizes the rings from cluster sizes). */
    readonly nodeArea: number;
    /** Fraction of free space between neighbouring clusters on a ring. */
    readonly gap: number;
    /** Extra clearance between the inner and the outer ring, world units. */
    readonly outerClearance: number;
    /** Smallest inner ring radius, world units. */
    readonly minRadius: number;
    /**
     * Stiffness factor for edges whose ends descend from different roots (e.g. a second
  ...
  /** Sleep when the mean kinetic energy per particle stays below `meanKE` for `steps` steps. */
  readonly sleep: { readonly meanKE: number; readonly steps: number };
  /** Glow decay: dE/dt = -E / tau. Energies below `floor` snap to 0. */
  readonly glow: { readonly tau: number; readonly floor: number };
}

/** Every physics constant, in one place. */
export const PHYSICS: PhysicsConfig = Object.freeze({
  dt: 1 / 120,
  maxStepsPerFrame: 4,
  maxFrameSeconds: 0.1,
  frameBudgetMs: 10,
 ...
 */
function kernel(qj: number, r2: number, eps2: number, r1sq: number, rcsq: number, invDen: number): number {
  let f = qj / (r2 + eps2);
  if (r2 > r1sq) {
    const a = rcsq - r2;
...
function cutoffTerms(p: RepulsionParams): { r1sq: number; rcsq: number; invDen: number } {
  if (!Number.isFinite(p.cutoffOuter)) return { r1sq: Infinity, rcsq: Infinity, invDen: 0 };
 ...
 */
function splitAngle(i: number, j: number): number {
  const lo = i < j ? i : j;
  const hi = i < j ? j : i;
  return ((lo * 0.6180339887 + hi * 0.4142135623) % 1) * TAU;
}
function splitX(i: number, j: number): number {
  return (i < j ? -1e-3 : 1e-3) * Math.cos(splitAngle(i, j));
}
function splitY(i: number, j: number): number {
...
export function repulsionBrute(
  x: Float64Array,
  y: Float64Array,
  q: Float64Array,
  n: number,
...
export interface AdvanceResult {
  /** Fixed steps consumed (dynamics ran only if awake). */
  readonly steps: number;
  /** True when positions changed. */
  readonly moved: boolean;
  /** True when the accumulator backlog was dropped (spiral-of-death clamp). */
...
export class Sim {
  ...
  readonly centroidX: Float64Array;
  readonly centroidY: Float64Array;
  /** Mass-weighted RMS distance of each cluster's particles from its centroid. */
  readonly clusterRadius: Float64Array;
  private readonly centroidMass: Float64Array;
  ...
  readonly ek: Float64Array;
  /** Per edge: stretch beyond which tension is constant. */
  readonly ereach: Float64Array;
  ...
  private readonly repulsionParams: RepulsionParams;

  constructor(graph: SimGraph, cfg: PhysicsConfig = PHYSICS) {
    this.cfg = cfg;
    ...
    for (const d of keep) if (d.k === "broader") parents[d.s]++;
    for (let e = 0; e < m; e++) {
      const d = keep[e];
    ...
    for (let i = 0; i < n; i++) if (!index.has(this.ids[i])) index.set(this.ids[i], i);
    const schemeCount = new Map<string, number>();
    for (const d of nodes) schemeCount.set(d.scheme, (schemeCount.get(d.scheme) ?? 0) + 1);
    let primary = cfg.ring.primaryScheme;
    if (!schemeCount.has(primary)) {
      let best = -1;
      for (const [s, c] of schemeCount) if (c > best || (c === best && s < primary)) [primary, best] = [s, c];
    }
    const byId = (a: number, b: number): number => (this.ids[a] < this.ids[b] ? -1 : this.ids[a] > this.ids[b] ? 1 : a - b);
    const inner: number[] = [];
    const outer: number[] = [];
    for (let i = 0; i < n; i++) {
    ...
    const blob = (slot: number): number => Math.sqrt((Math.max(1, clusterSize[slot]) * cfg.ring.nodeArea) / Math.PI);
    let sumInner = 0;
    let maxInner = 0;
    ...
    let meanBlob = 0;
    for (let s = 0; s < roots.length; s++) meanBlob += blob(s) / roots.length;
    ...
    for (let e = 0; e < m; e++) {
      ...
      if (a !== b) this.ek[e] *= cfg.ring.crossRootFactor;
    }
   ...
   */
  private place(inner: number[], outer: number[]): void {
    const n = this.n;
    const placed = new Uint8Array(n);
    const putRing = (list: number[], radius: number): void => {
      list.forEach((r, k) => {
    ...
    for (let e = 0; e < this.m; e++) {
      if (this.ekind[e] !== KIND_BROADER) continue;
      const s = this.es[e];
      const t = this.et[e];
      const p = parent[s];
      if (p === -1 || this.depth[t] < this.depth[p] || (this.depth[t] === this.depth[p] && this.ids[t] < this.ids[p])) parent[s] = t;
    }
    for (let s = 0; s < this.roots.length; s++) parent[this.roots[s]] = -1;
    const kids: number[][] = Array.from({ length: n }, () => []);
    for (let i = 0; i < n; i++) if (parent[i] >= 0) kids[parent[i]].push(i);
    for (const list of kids) list.sort((a, b) => (this.ids[a] < this.ids[b] ? -1 : 1));
    ...
    for (let k = post.length - 1; k >= 0; k--) {
      const v = post[k];
      size[v] += 1;
      if (parent[v] >= 0 && seen[parent[v]]) size[parent[v]] += size[v];
    }
    ...
    for (let s = 0; s < this.roots.length; s++) {
      ...
      const levels: number[][] = [[r]];
      for (let L = 0; L < levels.length && L < 64; L++) {
        const nextLevel: number[] = [];
        for (const v of levels[L]) for (const c of kids[v]) if (!placed[c] && seen[c]) nextLevel.push(c);
      ...
      const queue: Array<[number, number, number, number]> = [[r, phase, TAU, 0]];
      for (let q = 0; q < queue.length; q++) {
        const [v, a0, span, level] = queue[q];
        const list = kids[v].filter((c) => !placed[c]);
        let total = 0;
        for (const c of list) total += size[c] > 0 ? size[c] : 1;
        let a = a0;
        for (const c of list) {
  ...
  updateCentroids(): void {
    ...
    for (let s = 0; s < R; s++) rr[s] = sm[s] > 0 ? Math.sqrt(rr[s] / sm[s]) : 0;
  }
  ...
  grab(i: number, wx: number, wy: number): void {
  ...
  step(): void {
    ...
    for (let e = 0; e < this.m; e++) {
      if (ekind[e] === KIND_SEMANTIC && !semantic) continue;
      const s = es[e];
      const t = et[e];
      let dx = x[t] - x[s];
      let dy = y[t] - y[s];
      ...
      if (stretch > ereach[e]) stretch = ereach[e];
   ...
   */
  advance(frameSeconds: number): AdvanceResult {
    const cfg = this.cfg;
    const dt = cfg.dt;
