...
import type { Sim } from "./physics.js";
import { KIND_BROADER, KIND_SEMANTIC } from "./physics.js";
import type { Camera } from "./camera.js";
import type { Theme } from "./theme.js";
import type { GraphNode } from "./data.js";
import { RectGrid } from "./grid.js";

export type Lens = "structure" | "coverage";
...
export const RENDER = Object.freeze({
  ...
  depthAlpha: [1, 0.84, 0.7, 0.58] as readonly number[],
...
export interface Emphasis {
  ...
  selected: number;
  hovered: number;
  /** 1 for direct neighbours of the selected node. */
  ...
  obstacles: ReadonlyArray<readonly [number, number, number, number]>;
}
...
export class Renderer {
  ...
  private readonly radiusW: Float32Array;
  private readonly covBin: Int8Array;
  readonly langsMin: number;
  readonly langsMax: number;
  readonly langsKnown: number;
  private readonly labelText: string[];
  /** Label widths at the label font and at the 13 px domain font (scaled for other sizes). */
  private readonly labelW: Float32Array;
  private readonly rootW: Float32Array;
  private readonly massOrder: Int32Array;
  private readonly primaryRootSlot: Uint8Array;
  private readonly keys: Uint16Array;
  private readonly counts: Int32Array;
  private readonly order: Int32Array;
  private readonly grid = new RectGrid();
  private readonly trial = new RectGrid();
  ...
  constructor(canvas: HTMLCanvasElement, nodes: readonly GraphNode[], sim: Sim, theme: Theme, primaryScheme: string) {
    ...
    for (let s = 0; s < sim.roots.length; s++) this.primaryRootSlot[s] = nodes[sim.roots[s]!]!.scheme === primaryScheme ? 1 : 0;
  ...
  resize(): boolean {
    ...
    if (bw === this.canvas.width && bh === this.canvas.height && dpr === this.dpr && w === this.width && h === this.height) return false;
  ...
  private fontSize(kind: number): number {
    if (kind === F_ROOT) return this.rootSize;
    if (kind === F_OTHER_ROOT) return Math.max(10, this.rootSize - 1);
    if (kind === F_SELECTED) return RENDER.selectedFontSize;
    return RENDER.labelFontSize;
  }

  private font(kind: number): string {
  ...
  private measure(i: number, kind: number): number {
  ...
  draw(sim: Sim, cam: Camera, em: Emphasis): void {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (hits !== null && hits[i] === 1 && r < RENDER.hitMinRadius) r = RENDER.hitMinRadius;
      if ((i === sel || i === em.hovered) && r < RENDER.selectedMinRadius) r = RENDER.selectedMinRadius;
      sx[i] = px;
      sy[i] = py;
      sr[i] = r;
    ...
    if (em.glow && sim.glowing > 0) this.drawGlow(sim, sel);
    ...
    ctx.globalAlpha = 1;
    if (em.lens === "coverage" && hits) this.ringBatch((i) => hits[i] === 1 && vis[i] === 1, th.ink, 1.5);
    if (em.hovered >= 0 && vis[em.hovered] && em.hovered !== sel) {
      const hv = em.hovered;
      this.ringBatch((i) => i === hv, em.lens === "coverage" ? th.ink : th.accent, 1.5);
    }
    if (sel >= 0 && vis[sel]) this.ringBatch((i) => i === sel, th.accent2, RENDER.ringWidth);
  ...
  private drawEdges(sim: Sim, em: Emphasis, W: number, H: number): void {
    const ctx = this.ctx;
    const { sx, sy } = this;
    const es = sim.es;
    const et = sim.et;
  ...
  private drawGlow(sim: Sim, sel: number): void {
    const ctx = this.ctx;
    const E = sim.energy;
    const { sx, sy, sr, vis } = this;
    const levels = 8;
  ...
  private alphaOf(level: number): number {
    const d = RENDER.depthAlpha;
    if (level < 4) return d[level]!;
    if (level < 8) return d[level - 4]! * RENDER.searchMuted;
    if (level < 12) return d[level - 8]! * RENDER.selectionMuted;
    if (level === A_FULL) return 1;
    if (level === A_SEL_MUTED) return RENDER.selectionMuted;
    return RENDER.searchMuted;
  }

  private colorOf(c: number): string {
    const th = this.theme;
    if (c === C_INK) return th.ink;
    if (c === C_ACCENT) return th.accent;
    if (c === C_ACCENT2) return th.accent2;
    if (c === C_HOLLOW) return th.muted;
    return th.ramp[c - C_RAMP] ?? th.ink;
  }

  private drawParticles(sim: Sim, em: Emphasis): void {
    const ctx = this.ctx;
    ...
    for (let k = 1; k <= KEYS; k++) counts[k]! += counts[k - 1]!;
  ...
  private ringBatch(test: (i: number) => boolean, color: string, width: number): void {
    const ctx = this.ctx;
    const { sx, sy, sr } = this;
    const TAU = Math.PI * 2;
    ctx.beginPath();
   ...
   */
  private placeDomains(sim: Sim, fixed: readonly Rect[], em: Emphasis, W: number, H: number, size: number): Array<{ slot: number; rect: Rect; x: number; y: number }> | null {
    this.rootSize = size;
    const trial = this.trial;
    trial.reset(W, H, 48);
    for (const r of em.obstacles) trial.add(r[0], r[1], r[2], r[3]);
    for (const r of fixed) trial.add(r[0], r[1], r[2], r[3]);
  ...
  private drawLabels(sim: Sim, em: Emphasis, W: number, H: number): void {
    const th = this.theme;
    const grid = this.grid;
    grid.reset(W, H, 48);
    for (const r of em.obstacles) grid.add(r[0], r[1], r[2], r[3]);
    ...
    const byParticle = (i: number, kind: number, color: string, force: boolean): boolean => {
      if (!vis[i]) return false;
      const w = this.measure(i, kind);
      const h = this.fontSize(kind) * 1.2;
      const cy = sy[i]!;
      const gap = sr[i]! + (i === sel ? RENDER.ringGap + RENDER.ringWidth + 4 : 4);
      const right: Rect = [sx[i]! + gap - pad, cy - h / 2 - pad, sx[i]! + gap + w + pad, cy + h / 2 + pad];
      const left: Rect = [sx[i]! - gap - w - pad, cy - h / 2 - pad, sx[i]! - gap + pad, cy + h / 2 + pad];
      const inView = (r: Rect): boolean => r[0] >= 0 && r[2] <= W && r[1] >= 0 && r[3] <= H;
      const options: Array<[Rect, CanvasTextAlign]> = [
        [right, "left"],
        [left, "right"],
      ];
      for (const [rect, align] of options) {
        if (!force && (!inView(rect) || grid.collides(rect[0], rect[1], rect[2], rect[3]))) continue;
        if (force && !inView(rect) && rect === right && inView(left)) continue;
        grid.add(rect[0], rect[1], rect[2], rect[3]);
        if (force) fixed.push(rect);
    ...
    if (hasSel) byParticle(sel, F_SELECTED, th.ink, true);
    ...
    for (let slot = 0; slot < sim.roots.length; slot++) {
      if (this.primaryRootSlot[slot] !== 0) continue;
      const r = sim.roots[slot]!;
      if (r === sel) continue;
      const px = sim.centroidX[slot]! * this.s + this.ox;
      const py = sim.centroidY[slot]! * this.s + this.oy;
      ...
      if (grid.collides(rect[0], rect[1], rect[2], rect[3])) continue;
      grid.add(rect[0], rect[1], rect[2], rect[3]);
    ...
    if (em.hitOrder) {
      for (let k = 0; k < em.hitOrder.length && budget > 0; k++) {
        ...
        if (i === sel || sim.depth[i] === 0) continue;
        if (byParticle(i, F_LABEL, th.ink, false)) budget--;
      }
    }
    if (hasSel && nb) {
      for (let k = 0; k < this.n && budget > 0; k++) {
        const i = this.massOrder[k]!;
        if (nb[i] !== 1 || i === sel || sim.depth[i] === 0) continue;
        if (byParticle(i, F_LABEL, th.ink2, false)) budget--;
      }
    }
    if (!hits) {
      for (let k = 0; k < this.n && budget > 0; k++) {
        const i = this.massOrder[k]!;
        if (!vis[i] || sim.depth[i] === 0 || i === sel) continue;
        if (sr[i]! < RENDER.labelMinRadius) continue;
        if (hasSel && !(nb && nb[i] === 1)) continue;
        if (byParticle(i, F_LABEL, th.ink2, false)) budget--;
      }
    }
